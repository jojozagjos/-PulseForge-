...
function discoverTracks() {
  ...
  for (const dirEnt of fs.readdirSync(TRACKS_DIR, { withFileTypes: true })) {
    ...
    try {
      ...
      // add convenience absolute chart URLs if relative in manifest
      if (m.charts && typeof m.charts === "object") {
        for (const [k, v] of Object.entries(m.charts)) {
          if (typeof v === "string" && v.startsWith("./")) {
            m.charts[k] = `/tracks/${dirEnt.name}/${v.slice(2)}`;
          }
        }
      }

      // add convenience audio URLs if relative in manifest
      if (m.audio && typeof m.audio === "object") {
        for (const [k, v] of Object.entries(m.audio)) {
          if (typeof v === "string" && v.startsWith("./")) {
            m.audio[k] = `/tracks/${dirEnt.name}/${v.slice(2)}`;
          }
        }
      }
...
// ---------------- leaderboard (PG when available, else JSON file) ----------------

if (process.env.DATABASE_URL) {
  // ---- Postgres-backed router ----
  const { default: leaderboardRouter } = await import("./leaderboard.pg.js"); // server/leaderboard.pg.js
  app.use("/api/leaderboard", leaderboardRouter);
  console.log("[LB] Using Postgres leaderboard via DATABASE_URL");
} else {
  // ---- JSON file fallback (kept local under ./data/leaderboards.json) ----
  ...
  app.post("/api/leaderboard/submit", (req, res) => {
    ...
    const diff = (difficulty || "normal").toString();
  ...
  console.log("[LB] Using JSON file leaderboard fallback at ./data/leaderboards.json");
}
